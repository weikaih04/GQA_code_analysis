...
def filterOn(s, o):
    ...
    blacklist = [("train", "windshield"), ("containers", "spices"), ("cap", "oil"), ("cup", "flour"),
        ...
        ("home plate", "field"), ("sidewalk", "street")]
...
def filterWith(s, o):
    ...
    ("window", "curtains"), ("bowl", "food"), ("plate", "pizza"), ("man", "eye glasses"), ("man", "snowboard"), 
    ...
    ("woman", "eye glasses"), ("woman", "child"), ("woman", "coat"), ("person", "kite"), ("plant", "flowers"), 
    ...
    ("boy", "cap"), ("lady", "eye glasses"), ("lady", "umbrella"), ("cake", "candle"), ("box", "donut"), 
    ...
    ("woman", "handbag"), ("boy", "sunglasses"), ("boy", "eye glasses"), ("person", "sweater"), ("plate", "dessert"), 
    ...
    ("guy", "eye glasses"), ("guy", "camera"), ("guy", "earring"), ("container", "meal"), ("elephant", "chain"), 
...
def filterHave(s, o):
    whitelist = [("door", "window"), ("wall", "window"), ("bear", "shirt"), ("animal", "shirt"), ("alien", "shirt"), 
        ...
        ("boy", "boots"), ("man", "eye glasses"), ("woman", "eye glasses"), ("person", "eye glasses"), 
        ("lady", "eye glasses"), ("girl", "eye glasses"), ("boy", "eye glasses"), ("door", "lock"), ("cabinet", "lock"), 
...
wsd = {
    ...
    "old": {"age": ["young"], "4": ["new"]},
    "clear": {"weather": ["sunny", "cloudy"], "21": ["murky"], "opaqness": ["opaque"]},
...
def GetImageData(id=61512):
  ...
  # response = requests.get(image.url, verify=False, ssl_verify=False)
  # img = PIL_Image.open(io.BytesIO(response.content))  
  img = cv2.imread('../vg/vgi/{}.jpg'.format(id)) 
  return img
...
parser.add_argument('--hsideMax', default=0.48, type = float)
parser.add_argument('--vside', default=0.23, type = float) 
parser.add_argument('--vsideMax', default=0.37, type = float)
...
dataFilenameNew = "graphDataFinal/info{}.json"
dataFilenameShard = "graphDataFinal/info{}.json"
...
objCats = {
    ...
    "object": ['furniture', 'appliance', 'device', "toy", 'tableware', 'container', 'bottle', 'bag',
               'cooking utensil', 'footwear', 'accessory', 'instrument', 'weapon'],
...
def removeSubword(inputStr, blacklist):
    ...
    inputStr = inputStr.replace("made of", "madeof")
...
def globalleft(c):
    return midpoint(xrange(c)) < args.hside and max(xrange(c)) < (args.hsideMax)

def globalright(c):
    return midpoint(xrange(c)) > (1 - args.hside) and min(xrange(c)) > (1 - args.hsideMax)

def globaltop(c):
    return max(yrange(c)) < args.vsideMax # midpoint(yrange(c)) < args.vside

def globalbottom(c):
    return min(yrange(c)) > (1 - args.vsideMax)# midpoint(yrange(c)) > 1 - args.vside
...
def vis(imageId):   
    ...
    # fig = plt.gcf()
    # fig.set_size_inches(18.5, 10.5)
    objs = list(x[imageId]["objects"].values())
    plt.imshow(img)
    # ax = plt.gca()
    # print(len(objs))
    for obj in objs:
        ...
        for relId in obj["outRels"]:
            rel = obj["outRels"][relId]
            text += " " + "{}-{}".format(rel["rel"], x[imageId]["objects"][rel["obj"]]["name"])
        print(text)
...
def printGraph(vgData, imageId):
    print(imageId)
    for oId in vgData[imageId]["objects"]:
        o = vgData[imageId]["objects"][oId]
        objLine = o["name"]
        objLine += " A: ["
        ...
        for relId in o["outRels"]:
            ...
            if rel["rel"] not in ["to the right of", "to the left of"]:
                other = vgData[imageId]["objects"][rel["obj"]]
...
def normalize():
    ...
    translateDict = {
        ...
        "toilet roll": "roll",
        "tissue papers": "tissues",
        "vacuum cleaner": "vacuum",
        "electrical outlet": "outlet",
        "power outlet": "outlet",
        "electric outlet": "outlet",
        "erase board": "dry-erase board",
        "icecream": "ice cream",
        "sprinkle": "sprinkles",
    ...
    translateDictR = {
        ...
        "outside of": "outside",
        "on side": "on the side of",
        "surrounds": "surrounding",
        "beside of": "next to",
        "besides": "next to",
        "sits in": "sitting in",
        "carries": "carrying",
        "to right of": "to the right of",
        "to left of": "to the left of",
        ...
        "parked on side of": "parked on the side of",
    ...
    relListing = {
        "surrounding": [("w", "s", ["fence", "trees", "wall", "rope", "crust", "people", "border"])],
        ...
        "riding": [("b", "o", ["wave", "board", "waves", "lift", "water", "rail", "street", "subway", "hill", "skateboarder", "tracks", "benches", "horns", 
            ...
            "escalator", "beach", "foam", "jacket", "surfboard"]), ("b", "s", ["surfboard", "train", "wetsuit", "horse", "motorcycle", "car", "shirt", "shoes", "back", 
    ...
    for i, imageId in enumerate(data):
        ...
        for obj in instance["objects"].values():
            ...
            if obj["name"] in femaleO or "female" in obj["attributes"]:
                ...
                if obj["name"] in femaleO and "female" in obj["attributes"]:
                    obj["attributes"].remove("female")
            if obj["name"] in maleO or "male" in obj["attributes"]:
                #if "male" not in obj["mAttributes"]:
                obj["mAttributes"]["gender"] = "male"
                if obj["name"] in maleO and "male" in obj["attributes"]:
                    obj["attributes"].remove("male")
            ...
            if obj["name"] in healthyO or "healthy" in obj["attributes"]:
                ...
                # if obj["name"] in healthyO:
                #     obj["attributes"].append("healthy")
            if obj["name"] in unhealthyO or "unhealthy" in obj["attributes"]:              
                #if "male" not in obj["mAttributes"]:
                obj["mAttributes"]["healthiness"] = "unhealthy"
                # if obj["name"] in unhealthyO:
                #     obj["attributes"].append("unhealthy")  
        ...
        for obj in instance["objects"].values():
            ...
            for i, relId in enumerate(obj["outRels"]):
                ...
                if rel["rel"] in ["on top of", "on"] and isAlist(obj, ["food", "sauce"]) and isAlist(o, ["food", "container"]):
                    newRelId = "newW_" + str(idCounter)
                    idCounter += 1
                    newRel = {"rel": "with", "subj": rel["obj"], "obj": rel["subj"]}
                    toAdd.append((newRelId, newRel, o, obj))
                    rel["sym"] = newRelId
                    newRel["sym"] = relId
                ...
                if rel["rel"] == "covered":
                    ...
                    if max(c1,c2) > 0:
                        if c1 > c2:
                            ...
                            newRelId = "covered_" + str(idCounter)
                            idCounter += 1
                ...
                if rel["rel"] in relListing:
                    for condition in relListing[rel["rel"]]:
                        ...
                        if not cond:
                            # print(rel["rel"], rel["subj"], rel["obj"])
                            toDel.append(relId)
                ...
                if rel["rel"] in ["in front of", "behind"]:
                    if isA(obj, "part") or isA(o, "part") or isA(obj, "place"):
                        toDel.append(relId)
                ...
                if rel["rel"] == "on":
                    ...
                    if o == "ocean" and (s in ["surfer", "horse", "people", "man", "person"]):
                        rel["rel"] = "in"

                    if o == "ocean" and s == "bird":
                        rel["rel"] = "above"                        
            ...
            for item in toAdd:
                newRelId, newRel, s, o = item
                s["outRels"][newRelId] = newRel
                o["inRels"][newRelId] = newRel
        ...
        for objId in instance["objects"]:
            ...
            for otherObjId in instance["objects"]:
                ...
                if isLeft or isRight:
                    direction = "to the left of" if isLeft else "to the right of"
        ...
        if "predObjects" in instance:
            instance["name2obj"] = hashObjs(instance, instance["name2obj"], instance["predObjects"])
...
def createObjsCODict():
    ...
    for objName in oDict:
        for objName2 in oDict[objName]:
            cDict[objName][objName2] = float(oDict[objName][objName2]) / mDict[objName2]

        candidates = [c for c in oDict[objName].items()] # if c[1] < 0.9
        o1List[objName] = sorted(candidates, reverse = True, key = lambda x: x[1])[:50]   

        candidates = [c for c in cDict[objName].items()] # if c[1] < 0.9
        o2List[objName] = sorted(candidates, reverse = True, key = lambda x: x[1])[:50]
    ...
    return oDict, mDict, cDict, o1List, o2List, otList, otPerCatList
...
def createObjsSRODict():
    ...
    for i, instance in enumerate(data.values()):
        ...
        for subj in instance["objects"].values():
            ...
            for s,r in srCountO:
                if srCountO[(s,r)] > 2 and r not in ["to the left of", "to the right of"]:
                    srmultiODict[(s,r)] += 1
        ...
        for obj in instance["objects"].values():
            ...
            for o,r in orCountO:
                if orCountO[(o,r)] > 2 and r not in ["to the left of", "to the right of"]:                
                    ormultiODict[(o,r)] += 1    

        for s,r in srCount:
            if srCount[(s,r)] > 2 and r not in ["to the left of", "to the right of"]:
                srmultiDict[(s,r)] += 1
                
        for o,r in orCount:
            if orCount[(o,r)] > 2 and r not in ["to the left of", "to the right of"]:                
                ormultiDict[(o,r)] += 1
...
if args.normalize:
    # with open(dataFilenameNew, "w") as f:
    #    json.dump(data, f)
    with open(dataFilenameShard.format("Keys"), "w") as f:
       json.dump(list(data.keys()), f)   

    shardSize = int((float(len(data)) / 20) + 1)
    start = 0
    allData = data.items()
    for i in range(20):
        shard = dict(allData[start:(start + shardSize)])
        start += shardSize
        with open(dataFilenameShard.format(i), "w") as f:
           json.dump(shard, f)
else:
...
oaCount, oaProb, oaList, _ = createAttrsDict()
ooCount, oCount, ooProb, o1List, o2List, topOO, topCOO = createObjsCODict() # _,_,_,
...
with open(outJson("oCount"), "w") as outFile:
    json.dump(oCount, outFile)
...
with open(outJson("o1List"), "w") as outFile:
    json.dump(o1List, outFile)

with open(outJson("o2List"), "w") as outFile:
    json.dump(o2List, outFile)    
